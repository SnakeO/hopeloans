...
class ACA_WC_Column_Product_Sale extends AC_Column
	...
	private function get_scheduled_label( WC_Product $product ) {
		$labels = array(
			$this->format_scheduled_label( __( 'From', 'codepress-admin-columns' ), $product->get_date_on_sale_from() ),
			$this->format_scheduled_label( __( 'Until', 'codepress-admin-columns' ), $product->get_date_on_sale_to() ),
		);
