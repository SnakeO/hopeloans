...
class ACA_WC_Column_User_Orders extends AC_Column
	...
	public function get_value( $user_id ) {
		...
		foreach ( $order_ids as $order_id ) {
			...
			$values[ $order->get_status() ][] = '<div class="order order-' . esc_attr( $order->get_status() ) . '" ' . ac_helper()->html->get_tooltip_attr( $this->get_order_tooltip( $order ) ) . '">' . ac_helper()->html->link( get_edit_post_link( $order_id ), $order->get_order_number() ) . '</div>';
		}
	...
	public function export() {
		return new ACA_WC_Export_User_Orders( $this );
	}
