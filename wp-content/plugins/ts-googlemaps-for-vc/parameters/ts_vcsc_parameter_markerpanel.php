    ...
    if (!class_exists('TS_Parameter_MarkerPanel')) {
        class TS_Parameter_MarkerPanel {
            ...
            function mapmarker_settings_field($settings, $value) {
                ...
                $dir 			= $GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginDir;
				...
				$output .= '<div id="ts-googlemap-markers-wrapper-' . $randomizer . '" class="ts-googlemap-markers-wrapper clearFixMe ts-settings-parameter-gradient-grey">';
					$output .= '<div id="ts-googlemap-markers-controls-' . $randomizer . '" class="ts-googlemap-markers-controls">';
						$output .= '<div id="ts-googlemap-markers-result-' . $randomizer . '" class="ts-googlemap-markers-result" data-path="' . TS_GMVC_GetResourceURL('images/marker/') . '">';
							$output .= '<img src="' . TS_GMVC_GetResourceURL('images/marker/') . ($value != "" ? $value : basename($images[0])) . '">';
						$output .= '</div>';
					...
					$output .= '<div id="ts-googlemap-markers-selector-' . $randomizer . '" class="ts-visual-selector ts-googlemap-markers-selector" style="max-height: ' . $height . 'px;">';				
						foreach($images as $img)     {
							...
							$counter++;
							if (($value == '') && ($counter == 1)) {
								$output .= '<a class="TS_VCSC_Marker_Link current" href="#" title="' . __( "Marker Name:", "ts_visual_composer_extend" ) . ': ' . $markername . '" rel="' . $markername . '"><img src="' . TS_GMVC_GetResourceURL('images/marker/') . $markername . '" style="height: 37px; width: 32px;"><div class="selector-tick"></div></a>';
							} else if ($value == $markername) {
								$output .= '<a class="TS_VCSC_Marker_Link current" href="#" title="' . __( "Marker Name:", "ts_visual_composer_extend" ) . ': ' . $markername . '" rel="' . $markername . '"><img src="' . TS_GMVC_GetResourceURL('images/marker/') . $markername . '" style="height: 37px; width: 32px;"><div class="selector-tick"></div></a>';
							} else {
								$output .= '<a class="TS_VCSC_Marker_Link" href="#" title="' . __( "Marker Name:", "ts_visual_composer_extend" ) . ': ' . $markername . '" rel="' . $markername . '"><img src="' . TS_GMVC_GetResourceURL('images/marker/') . $markername . '" style="height: 37px; width: 32px;"></a>';
							}						
						}					
					$output .= '</div>';
