    ...
    if (!class_exists('TS_Parameter_LinkPicker')) {
        class TS_Parameter_LinkPicker {
            ...
            function advancedlinks_settings_field($settings, $value) {
                ...
                // Post Retrieval
                $load_offset            = isset($settings['load_count']) ? $settings['load_count'] : $GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['offset'];
                $load_orderby           = isset($settings['load_orderby']) ? $settings['load_orderby'] : $GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['orderby'];
                $load_order             = isset($settings['load_order']) ? $settings['load_order'] : $GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['order'];
                // Text Strings
                ...
                // Parse Link Data
                $link_json              = TS_GMVC_Advancedlinks_BuildLinkArray($value);
                ...
                // Contingency Checks
                if (($link_source =='post') && ($GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['posts'] == "false")) {
                    $link_source        = 'page';
                } else if (($link_source =='custom') && ($GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['custom'] == "false")) {
                    $link_source        = 'page';
                }
                ...
                if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['custom'] == "true") {
                    $args = array(
                       'public'                 => true,
                ...
                $countPosts             = 0;
                $countCustom            = 0;
                if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['global'] == "false") {
                    ...
                    $output .= '<div id="ts-advancedlinks-settings-' . $randomizer . '" class="ts-advancedlinks-settings" style="display: none;">';
                        ...
                        $output .= '<div id="ts-advancedlinks-selector-holder-' . $randomizer . '" class="ts-advancedlinks-selector-holder" style="display: ' . ($target == "true" ? "inline-block;" : "block;") . '; ' . ($target == "true" ? " width: 48%; margin: 0 2% 0 0;" : " width: 100%; margin: 0;") . ' padding: 0;">';
                            ...
                            $output .= '<select name="ts-advancedlinks-selector-' . $randomizer . '" id="ts-advancedlinks-selector-' . $randomizer . '" class="ts-advancedlinks-selector" value="' . $link_source . '" style="">';
                                ...
                                if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_ParameterLinkPicker['posts'] == "true") {
