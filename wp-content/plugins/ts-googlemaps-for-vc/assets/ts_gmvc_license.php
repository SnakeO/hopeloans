<?php
	...
	// ------------------------------
	if (!class_exists('TS_GMVC_EnvatoAPIRoutines')) {
		class TS_GMVC_EnvatoAPIRoutines {
			// Private Class Variables
			private $envato_item							= "13510102";
			...
			function __construct($envato_item, $envato_token) {
				...
				$this->envato_protocol						= $this->TS_GMVC_HelperSiteProtocol() . '://';
				$this->envato_domain						= $this->envato_protocol . preg_replace('#^https?://#', '', site_url());
			}
			
			// Initialize License Code Routine
			function TS_GMVC_LicenseCodeInit($envato_type, $envato_code) {
			...
			function TS_GMVC_LicenseCodeNew($envato_code) {
				global $GOOGLEMAPS_PLUS_VC;
				if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginIsMultiSiteActive == "true") {
			...
			function TS_GMVC_LicenseCodeRemove() {
				global $GOOGLEMAPS_PLUS_VC;
				if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginIsMultiSiteActive == "true") {
			...
			// Retrieve Stored License Code
			function TS_GMVC_LicenseCodeRetrieval() {
				global $GOOGLEMAPS_PLUS_VC;
				if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginIsMultiSiteActive == "true") {
			...
			function TS_GMVC_LicenseCodeCheckup() {
				global $GOOGLEMAPS_PLUS_VC;
				if (!in_array(base64_encode($this->envato_code), $GOOGLEMAPS_PLUS_VC->TS_GMVC_Avoid_Duplications)) {
					if ((function_exists('curl_init')) && (strlen($this->envato_code) != 0)) {
						...
						$envato_agent						= $this->TS_GMVC_HelperRandomUserAgent();
			...
			function TS_GMVC_LicenseCodeSummary() {
				global $GOOGLEMAPS_PLUS_VC;				
				if ($this->envato_error != '') {
					...
					if ($this->envato_error != '') {
						$this->envato_message .= '<br><br/><strong>' . $this->TS_GMVC_HelperErrorsCURL($this->envato_error) . '</strong>';
					} else {
						...
						$this->envato_message .= '<div id="ts-license-check-confirmed" class="ts-license-check-message" data-key="' . $this->envato_code . '">';
							...
							$this->envato_message .= '<div class="clearFixMe">';						
								$this->envato_message .= '<div style="float: left;">';
									$this->envato_message .= '<img src="' . TS_GMVC_GetResourceURL('images/envato/envato_logo.png') . '" width="111" height="125" style="float: left; margin-right: 20px;">';
								...
								$this->envato_message .= '<div style="float: left;">';
									...
									$this->envato_message .= '<a href="https://codecanyon.net/item/google-maps-plus-for-visual-composer/13510102" target="_blank">' . __("Buy More Licenses", "ts_visual_composer_extend")  . '</a> | ';
			...
			// Store License Code Data
			function TS_GMVC_LicenceCodeStorage() {
				global $GOOGLEMAPS_PLUS_VC;
			...
			function TS_GMVC_ItemInfoInit() {
				...
				$this->TS_GMVC_ItemInfoSummary();
				// Return HTML Output
				return $this->envato_message;
			}
			
			// Retrieve Item Information from Envato
			function TS_GMVC_ItemInfoSummary() {
				global $GOOGLEMAPS_PLUS_VC;
				if (isset($GOOGLEMAPS_PLUS_VC->TS_GMVC_Envato_Globals['migrate'])) {
					$envato_migrate 						= $GOOGLEMAPS_PLUS_VC->TS_GMVC_Envato_Globals['migrate'];
				} else if (isset($GOOGLEMAPS_PLUS_VC->TS_GMVC_Envato_Defaults['migrate'])) {
					$envato_migrate 						= $GOOGLEMAPS_PLUS_VC->TS_GMVC_Envato_Defaults['migrate'];
				} else {
				...
				if (($this->envato_response == false) || ($this->envato_response == 'false')) {
					...
					// Parse Item Data					
					$TS_GMVC_Envato_Item_Name     						= (isset($this->envato_response["name"]) 				? $this->envato_response["name"] 				: "N/A");
					$TS_GMVC_Envato_Item_User							= (isset($this->envato_response["author_username"]) 	? $this->envato_response["author_username"] 	: "N/A");
					$TS_GMVC_Envato_Item_Rating							= (isset($this->envato_response["rating"]["rating"]) 	? $this->envato_response["rating"]["rating"] 	: "N/A");
					$TS_GMVC_Envato_Item_Votes							= (isset($this->envato_response["rating"]["count"]) 	? $this->envato_response["rating"]["count"] 	: "N/A");
					$TS_GMVC_Envato_Item_Sales							= (isset($this->envato_response["number_of_sales"]) 	? $this->envato_response["number_of_sales"] 	: "N/A");
					$TS_GMVC_Envato_Item_Price							= (isset($this->envato_response["price_cents"]) 		? $this->envato_response["price_cents"] 		: "N/A");
					$TS_GMVC_Envato_Item_Thumb							= (isset($this->envato_response["thumbnail_url"]) 		? $this->envato_response["thumbnail_url"] 		: "N/A");
					$TS_GMVC_Envato_Item_Link							= (isset($this->envato_response["url"]) 				? $this->envato_response["url"] 				: "N/A");
					$TS_GMVC_Envato_Item_Release						= (isset($this->envato_response["published_at"]) 		? $this->envato_response["published_at"] 		: "N/A");
					$TS_GMVC_Envato_Item_Update							= (isset($this->envato_response["updated_at"]) 			? $this->envato_response["updated_at"] 			: "N/A");
					$TS_GMVC_Envato_Item_Check							= time();
					// Populate Data Array
					$envato_array							= array();
					$envato_array["name"] 					= $TS_GMVC_Envato_Item_Name;
					$envato_array["author_username"]		= $TS_GMVC_Envato_Item_User;
					$envato_array["rating"]["rating"]		= $TS_GMVC_Envato_Item_Rating;
					$envato_array["rating"]["count"]		= $TS_GMVC_Envato_Item_Votes;
					$envato_array["number_of_sales"]		= $TS_GMVC_Envato_Item_Sales;
					$envato_array["price_cents"]			= $TS_GMVC_Envato_Item_Price;
					$envato_array["thumbnail_url"]			= $TS_GMVC_Envato_Item_Thumb;
					$envato_array["url"]					= $TS_GMVC_Envato_Item_Link;
					$envato_array["published_at"]			= $TS_GMVC_Envato_Item_Release;
					$envato_array["updated_at"]				= $TS_GMVC_Envato_Item_Update;
					// Create HTML Output					
					$this->envato_message .= '<div class="ts-envato-item-info-wrapper">';
						$this->envato_message .= '<div class="ts-envato-item-info-title">' . $TS_GMVC_Envato_Item_Name . '</div>';
						$this->envato_message .= '<div class="ts-envato-item-info-main">';
							$this->envato_message .= '<div class="ts-envato-item-info-top">';
								$this->envato_message .= '<div class="ts-envato-item-info-rating"><span class="ts-envato-item-info-desc">' . __( "Rating", "ts_visual_composer_extend" ) . '</span>' . $this->TS_GMVC_ItemInfoRating(round($TS_GMVC_Envato_Item_Rating)) . '</div>';
							$this->envato_message .= '</div>';
							$this->envato_message .= '<div class="ts-envato-item-info-middle">';
								$this->envato_message .= '<div class="ts-envato-item-info-sales">';
									...
									$this->envato_message .= '<div class="ts-envato-item-info-text">';
										$this->envato_message .= '<span class="ts-envato-item-info-num">' . number_format(floatval($TS_GMVC_Envato_Item_Sales), 0) . '</span>';
								...
								$this->envato_message .= '<div class="ts-envato-item-info-thumb">';
									$this->envato_message .= '<img src="' . $TS_GMVC_Envato_Item_Thumb . '" alt="' . $TS_GMVC_Envato_Item_Name . '" width="80" height="80"/>';
								$this->envato_message .= '</div>';
								$this->envato_message .= '<div class="ts-envato-item-info-price">';
									$this->envato_message .= '<span class="ts-envato-item-info-imgprice"></span>';
									$this->envato_message .= '<div class="ts-envato-item-info-text">';
										$this->envato_message .= '<span class="ts-envato-item-info-num"><span>$</span>' . round($TS_GMVC_Envato_Item_Price / 100) . '</span>';
							...
							$this->envato_message .= '<div class="ts-envato-item-info-bottom">';
								$this->envato_message .= '<div class="ts-advanced-link-button-wrapper ts-advanced-link-tooltip-holder" style="display: inline-block;">';
									...
									$this->envato_message .= '<a href="' . $TS_GMVC_Envato_Item_Link . '" target="_blank" class="ts-advanced-link-button-main ts-advanced-link-button-orange ts-advanced-link-button-purchase">' . __( "Purchase", "ts_visual_composer_extend" ) . '</a>';
								$this->envato_message .= '</div>';
							$this->envato_message .= '</div>';
						$this->envato_message .= '</div>';
					$this->envato_message .= '</div>';
					...
					if (($this->envato_success) && ($this->envato_response != false)) {
						$GOOGLEMAPS_PLUS_VC->TS_GMVC_Envato_Globals	= array(
							'last'										=> $TS_GMVC_Envato_Item_Check,
							'data'										=> $envato_array,
							'name'										=> $TS_GMVC_Envato_Item_Name,
							'info'										=> $this->envato_message,
							'link'										=> $TS_GMVC_Envato_Item_Link,
							'price'										=> $TS_GMVC_Envato_Item_Price,
							'sales'										=> $TS_GMVC_Envato_Item_Sales,
							'rating'									=> $this->TS_GMVC_ItemInfoRating($TS_GMVC_Envato_Item_Rating),
							'votes'										=> $TS_GMVC_Envato_Item_Votes,
							'check'										=> $TS_GMVC_Envato_Item_Check,
							'migrate'									=> true,
						);
						update_option("ts_vcsc_extend_settings_envato", $GOOGLEMAPS_PLUS_VC->TS_GMVC_Envato_Globals);
					}
				}
			}
			
			// Create HTML Output for Rating Stars
			function TS_GMVC_ItemInfoRating($envato_rating) {
			...
			function TS_GMVC_HelperRandomUserAgent() {
				$envato_agents 						= array(
			...
			function TS_GMVC_HelperSiteProtocol() {
			...
			function TS_GMVC_HelperErrorsCURL($envato_error) {
				$envato_errors = array (
					0 	=> 'CURLE_OK',
	...
	// ---------------------------
	$TS_GMVC_EnvatoClassInit 								= new TS_GMVC_EnvatoAPIRoutines("13510102", "QnFOMlI0b3NOU1ZiTTBmS3VOTjNBbFJHdk53c3Q2RUY=");
	...
	if (isset($_POST['License'])) {
		...
		$TS_GMVC_EnvatoClassInit->TS_GMVC_LicenseCodeInit("newlicense", trim($_POST['ts_vcsc_extend_settings_license']));
		// Reload License Page
		...
		$TS_GMVC_EnvatoClassInit->TS_GMVC_LicenseCodeInit("unlicense", "");
		// Reload License Page
		echo '<script> window.location="' . $_SERVER['REQUEST_URI'] . '"; </script> ';
		// Stop All
		Exit();
		...
		if ($ts_vcsc_extend_settings_licenseUpdate == 1) {
			$TS_GMVC_EnvatoClassInit->TS_GMVC_LicenseCodeInit("checklicense", "");
		} else {
			$TS_GMVC_EnvatoClassInit->TS_GMVC_LicenseCodeInit("getlicense", "");
		}
	...
	echo '<div class="ts-vcsc-settings-group-header">';
		echo '<div class="display_header">';
			echo '<h2><span class="dashicons dashicons-admin-network"></span>Composium - Visual Composer Extensions v' . TS_GMVC_GetPluginVersion() . ' ... ' . __("License Information", "ts_visual_composer_extend") . '</h2>';
		echo '</div>';
...
<form id="ts-vcsc-license-check-wrap" class="ts-vcsc-license-check-wrap" name="oscimp_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
	...
	<div class="ts-vcsc-section-main">
		...
		<div class="ts-vcsc-section-content">
			<?php
				if (current_user_can('manage_options')) {
					echo '<div class="ts-advanced-link-button-wrapper ts-advanced-link-tooltip-holder" style="margin: 10px 0;">
						...
						<a href="' . $GOOGLEMAPS_PLUS_VC->settingsLink . '" target="_parent" class="ts-advanced-link-button-main ts-advanced-link-button-grey ts-advanced-link-button-settings">'. __("Back to Settings", "ts_visual_composer_extend") . '</a>
					</div>';
				}
				if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginValid == "false") {
			...
			<?php
				...
				if (in_array(base64_encode($TS_GMVC_EnvatoClassInit->envato_code), $GOOGLEMAPS_PLUS_VC->TS_GMVC_Avoid_Duplications)) {
					echo '<div class="ts-vcsc-info-field ts-vcsc-critical" style="margin-top: 10px; font-size: 13px; text-align: justify; font-weight: bold;">';
		...
		<table style="border: 1px solid #ededed; min-height: 100px; width: 100%;">
			<tr>
				<td style="width: 250px; padding: 0px 20px 0px 20px; border-right: 1px solid #ededed;">
					<?php echo $TS_GMVC_EnvatoClassInit->TS_GMVC_ItemInfoInit(); ?>
				</td>
				<td>
					<div>
						...
						<?php echo $TS_GMVC_EnvatoClassInit->envato_status; ?>
						...
						<?php
							if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginIsMultiSiteActive == "true") {
								if (strlen($TS_GMVC_EnvatoClassInit->envato_code) != 0) {
	...
	<div class="ts-vcsc-extend-license-controls" style="width: 100%; margin-top: 20px;">		
		<?php
			if (function_exists('curl_init')) {
				if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginIsMultiSiteActive == "true") {
					if ($GOOGLEMAPS_PLUS_VC->TS_GMVC_PluginValid == "true") {
